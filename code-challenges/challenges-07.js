...
function chkRejected(rejected) {
    if (rejected.yearsOfExperience <= 1) {
        return true;
    }
    else if ((rejected.firstName === null || rejected.firstName == "") && (rejected.lastName === null || rejected.lastName == "")) {
        return true;
    }
    else {
        return false;
...
const classesAvg = (data) => {
    for (let i = 0; i < data.grades.length; i++) {
        for (let j = 0; j < data.grades[i].classes.length; j++) {
            for (let k = 0; k < data.grades[i].classes[j].length; k++) {
                let temp = 0;
                for (let z = 0; z < data.grades[i].classes[j].classScores.length; z++) {
                    temp += data.grades[i].classes[j].classScores[z];
                }
                if (temp > 0) {
                    data.grades[i].classes[j].avg = Math.floor(temp / data.grades[i].classes[j].classScores.length);
                }
            }
        }
    }
